...
?>
        ...
        <div class="page-title">
            <div class="title_left">
                <h3>
					<small>Accueil /</small> Rapport Individuel
                </h3>
            </div>
        </div>
...
<div class="row">
	...
	<div class="col-md-3">
						...
						<form method="post" action="">
										...
										<option value="0">Sélectionner le Numéro de Membre</option>
										<?php
										$result= mysqli_query($con,"select * from user") or die (mysqli_error($con));
										while ($row= mysqli_fetch_array ($result) ){
										$id=$row['user_id'];
										?>
                                            <option value="<?php echo $row['roll_number']; ?>">
                                                <?php echo $row['roll_number']; ?> - <?php echo $row['firstname']; ?>
                                            </option>
										<?php } ?>
                                        </select>  
				<br />
				<br />
						<button name="submit" type="submit" class="btn btn-primary" style="margin-left:110px;">
                            <i class="glyphicon glyphicon-log-in"></i> Valider
                        </button>
						</form>
...
<?php
	...
	$row = mysqli_fetch_array($sql);
		...
		if($count <= 0){
			echo "<div class='alert alert-success'>".'Aucun résultat trouvé pour le Numéro d’Identification'."</div>";
		}else{
			$roll_number = $_POST['roll_number'];
